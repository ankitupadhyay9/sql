...
from 
(
select id, company, @row_number := case when @company = e.company then @row_number+1 else 1 end as rn, @company := e.company, salary, (select count(1) from employee e2 where e2.company = e.company) total_emp
from employee e
order by company, salary
) t1
where (MOD(total_emp,2) = 0 and (rn = total_emp/2 or rn = total_emp/2+1))
or
(MOD(total_emp,2) <> 0 and (rn = CEIL(total_emp/2)))
